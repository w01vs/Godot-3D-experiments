...
func _validate_property(property: Dictionary) -> void:
	enable_if(property, "time_per_tick", type == Type.DOT)
	enable_if(property, "change_per_tick", type == Type.DOT)
	enable_if(property, "ticks", type == Type.DOT)
	enable_if(property, "health_change_total", type == Type.INSTANT)
	enable_if(property, "damage_multiplier", type == Type.INSTANT)
